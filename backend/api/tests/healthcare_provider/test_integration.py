...
from api.models import HealthcareProvider, ProviderHospitalAssignment, Appointment, Slot
...
class TestHealthcareProviderFlow:
    def test_provider_creation_and_onboarding_flow(
        self, authenticated_admin_client, speciality_factory, hospital_factory
    ):
        ...
        login_data = {"email": "dr.smith@example.com", "password": "ComplexPass123!"}
    ...
    def test_provider_hospital_affiliation_management(
        self,
        ...
        ProviderHospitalAssignment.objects.filter(healthcare_provider=provider).delete()

        # Assign provider to hospitals
        assign_url = reverse("provider-assign-hospital", args=[provider.pk])

        response = admin_client.post(
            assign_url, {"hospital_id": hospital1.id}, format="json"
        )
        assert response.status_code == status.HTTP_201_CREATED

        response = admin_client.post(
            assign_url, {"hospital_id": hospital2.id}, format="json"
        )
        assert response.status_code == status.HTTP_201_CREATED

        # Verify hospital affiliations
        active_assignments = ProviderHospitalAssignment.objects.filter(
            healthcare_provider=provider, is_active=True
        )
        ...
        response = admin_client.post(
            unassign_url, {"hospital_id": hospital1.id}, format="json"
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_provider_slot_management_flow(
        self,
        ...
        params = {"provider": provider.user_id, "date": base_date.isoformat()}
    ...
    def test_provider_schedule_generation_endpoint(
        self, authenticated_provider_client, hospital_factory
    ):
        ...
        # Verify slots
        slots = Slot.objects.filter(healthcare_provider=provider, start__date=test_date)
        assert slots.count() > 0
        assert slots.first().duration == timedelta(minutes=45)

    def test_provider_search_and_filtering(
        self,
        authenticated_patient_client,
        provider_factory,
        speciality_factory,
    ...
    def test_provider_appointment_history(
        self, authenticated_provider_client, appointment_factory, patient_factory
    ):
        ...
        other_provider_appointment = appointment_factory(
            patient=patient1, status="CONFIRMED"
        )
    ...
    def test_provider_onboard_existing_user(
        self,
        authenticated_admin_client,
        user_factory,
        speciality_factory,
...
class TestHealthcareProviderProfileAccess:
    ...
    def test_provider_cannot_update_others_profile(
        self, provider_factory, authenticated_provider_client
    ):
...
class TestHealthcareProviderSlotValidation:
    ...
    def test_provider_cannot_create_slot_with_end_before_start(
        self, authenticated_provider_client
    ):
        ...
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert (
            "after start" in str(response.data).lower()
            or "end" in str(response.data).lower()
        )
...
class TestHealthcareProviderAppointmentManagement:
    ...
    def test_provider_confirm_appointment(
        self, authenticated_provider_client, authenticated_patient_client, slot_factory
    ):
        ...
        response = provider_client.post(
            set_status_url, {"status": "CONFIRMED"}, format="json"
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_provider_cancel_appointment(
        self,
        authenticated_provider_client,
        appointment_factory,
        patient_factory,
        ...
        response = provider_client.post(
            set_status_url, {"status": "CANCELLED"}, format="json"
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_provider_reschedule_appointment(
        self,
        authenticated_provider_client,
        appointment_factory,
        patient_factory,
...
class TestHealthcareProviderEdgeCases:
    ...
    def test_provider_invalid_status_transitions(
        self, authenticated_provider_client, authenticated_patient_client, slot_factory
    ):
        ...
        response = provider_client.post(
            set_status_url, {"status": "RESCHEDULED"}, format="json"
        )
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        ...
        response = provider_client.post(
            set_status_url, {"status": "REQUESTED"}, format="json"
        )
        assert response.status_code == status.HTTP_400_BAD_REQUEST

    def test_provider_reschedule_to_unavailable_slot(
        self, authenticated_provider_client, authenticated_patient_client, slot_factory
    ):
        ...
        original_slot.refresh_from_db()
        assert original_slot.status == Slot.Status.BOOKED, (
            f"Slot status is {original_slot.status}, expected BOOKED"
        )
