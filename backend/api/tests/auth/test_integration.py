...
class TestAuthenticationFlow:
    ...
    def test_user_registration_and_verification_flow(self, mailoutbox):
        ...
        token_match = re.search(
            r'href=[\'"]?[^\'">]*/verify-email\?token=([^\'"\s>]+)', html_content
        )
    ...
    def test_password_change_flow(self, authenticated_user_client):
        ...
        login_url = reverse("login")
        login_data = {"email": user.email, "password": "NewComplexPass123!"}
        response = client.post(login_url, login_data, format="json")
        assert response.status_code == status.HTTP_200_OK

        old_login_data = {"email": user.email, "password": "complex123!"}
    ...
    def test_logout_flow(self, authenticated_user_client):
        ...
        login_url = reverse("login")
        login_data = {"email": user.email, "password": "complex123!"}
        response = client.post(login_url, login_data, format="json")
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_invalid_login_attempts(self, user_factory):
        ...
        for scenario in scenarios:
            ...
            if scenario.get("expected_msg"):
                assert (
                    scenario["expected_msg"].lower() in response.data["detail"].lower()
                )

    def test_provider_patient_onboarding_after_verification(
        self, user_factory, speciality_factory
    ):
