...
class TestPatientRegistrationFlow:
    ...
    def test_patient_standalone_signup_flow(self, mailoutbox):
        ...
        login_response = api_client.post(
            login_url,
            {"email": "new.patient@example.com", "password": "Complex123!"},
            format="json",
        )
    ...
    def test_patient_onboarding_validation(self, user_factory):
        ...
        # Test invalid weight
        response = api_client.post(url, {"weight": -10, "height": 175}, format="json")
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert "greater than or equal to 0" in str(response.data).lower()

        # Test invalid height
        response = api_client.post(url, {"weight": 70, "height": -5}, format="json")
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert "greater than or equal to 0" in str(response.data).lower()
...
class TestPatientProfileFlow:
    ...
    def test_patient_retrieve_own_profile(self, patient_factory):
        ...
        patient = patient_factory(blood_type="B+", allergies="Dust", weight=80)
        api_client = APIClient()
    ...
    def test_patient_update_own_profile(self, patient_factory):
        ...
        patient = patient_factory(blood_type="O+", allergies="None", weight=70)
        api_client = APIClient()
        ...
        url = reverse("patient-detail", kwargs={"pk": patient.user.pk})
        update_data = {"blood_type": "A+", "allergies": "Peanuts", "weight": 72}
...
class TestPatientAppointmentFlow:
    ...
    def test_patient_view_own_appointments(
        self, patient_factory, appointment_factory, provider_factory
    ):
        ...
        appointment_factory(patient=other_patient, healthcare_provider=provider)
    ...
    def test_patient_view_single_appointment(
        self, patient_factory, appointment_factory
    ):
    ...
    def test_patient_book_appointment_with_free_slot(
        self, patient_factory, provider_factory, slot_factory
    ):
    ...
    def test_patient_cannot_book_past_appointment(
        self, patient_factory, provider_factory
    ):
        ...
        appointment_data = {
            ...
            "appointment_end_datetime_utc": (
                past_time + timedelta(minutes=30)
            ).isoformat(),
            "reason": "Should fail",
        }
    ...
    def test_patient_cannot_book_already_booked_slot(
        self, patient_factory, provider_factory, slot_factory
    ):
        ...
        # Create booked slot
        slot = slot_factory(
            healthcare_provider=provider, hospital=hospital, status=Slot.Status.BOOKED
        )
    ...
    def test_patient_view_free_slots(
        self, patient_factory, provider_factory, slot_factory
    ):
        ...
        # Booked slot
        booked_start = timezone.make_aware(datetime.combine(base_date, time(14, 0)))
        slot_factory(
            healthcare_provider=provider,
            hospital=hospital,
            start=booked_start,
...
class TestPatientMedicalRecordFlow:
    ...
    def test_patient_view_own_medical_records(
        self, patient_factory, medical_record_factory
    ):
    ...
    def test_patient_view_single_medical_record(
        self, patient_factory, medical_record_factory
    ):
    ...
    def test_patient_cannot_view_others_medical_record(
        self, patient_factory, medical_record_factory
    ):
...
class TestPatientValidationFlow:
    ...
    def test_patient_me_endpoint(self, patient_factory):
        ...
        patient = patient_factory(blood_type="AB-", allergies="Shellfish")
        api_client = APIClient()
        ...
        update_data = {"blood_type": "O+", "allergies": "Updated allergies"}
        response = api_client.patch(url, update_data, format="json")
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_patient_appointment_constraints(
        self, patient_factory, provider_factory, slot_factory
    ):
