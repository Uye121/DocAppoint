...
class TestMedicalRecordCreateSerializer:
    ...
    def test_validate_authentication_required(
        self, patient_factory, hospital_factory, appointment_factory
    ):
    ...
    def test_validate_non_provider_user(
        self, patient_factory, hospital_factory, appointment_factory
    ):
    ...
    def test_validate_provider_removed(
    ...
    def test_validate_provider_cannot_create_for_self(
        ...
        with pytest.raises(Exception) as exc_info:
            serializer.is_valid(raise_exception=True)
        assert "Provider cannot create medical records for themselves" in str(
            exc_info.value
        )
    ...
    def test_create_medical_record_with_medical_record_factory(
        ...
        new_appointment = appointment_factory(
            patient=record.patient, healthcare_provider=provider
        )
    ...
    def test_create_with_multiple_hospital_affiliations(
        ...
        serializer1 = MedicalRecordCreateSerializer(
            data=data1, context={"request": request}
        )
        ...
        serializer2 = MedicalRecordCreateSerializer(
            data=data2, context={"request": request}
        )
        assert serializer2.is_valid() is True

    def test_serializer_context_required(
        self, patient_factory, hospital_factory, appointment_factory
    ):
        patient = patient_factory()
        hospital = hospital_factory()
...
class TestMedicalRecordUpdateSerializer:
    ...
    def test_update_medical_record_success(
        ...
        serializer = MedicalRecordUpdateSerializer(
            instance=record, data=data, context={"request": request}
        )
    ...
    def test_partial_update(self, medical_record_factory):
        ...
        serializer = MedicalRecordUpdateSerializer(
            instance=record, data=data, context={"request": request}, partial=True
        )
    ...
    def test_update_with_same_hospital(self, medical_record_factory):
        record = medical_record_factory()
        provider = record.healthcare_provider
        factory = APIRequestFactory()
        request = factory.put("/")
    ...
    def test_update_with_removed_provider(self, medical_record_factory):
        record = medical_record_factory()
        provider = record.healthcare_provider
        factory = APIRequestFactory()
        request = factory.put("/")
    ...
    def test_update_validation_errors(self, medical_record_factory):
        record = medical_record_factory()
        provider = record.healthcare_provider
        factory = APIRequestFactory()
        request = factory.put("/")
    ...
    def test_update_authentication_required(self, medical_record_factory):
        record = medical_record_factory()
    ...
    def test_update_with_removed_hospital(
        ...
        with pytest.raises(DRFValidationError):
            serializer.is_valid(raise_exception=True)
        assert "Selected hospital is no longer active" in str(
            serializer.errors.values()
        )

    def test_update_empty_notes_validation(self, medical_record_factory):
        record = medical_record_factory()
        provider = record.healthcare_provider
        factory = APIRequestFactory()
        request = factory.put("/")
    ...
    def test_update_empty_prescriptions_validation(self, medical_record_factory):
        record = medical_record_factory()
        provider = record.healthcare_provider
        factory = APIRequestFactory()
        request = factory.put("/")
        ...
        assert "This field may not be blank" in str(
            serializer.errors["prescriptions"][0]
        )

    def test_update_no_fields_changed(self, medical_record_factory):
        record = medical_record_factory()
        provider = record.healthcare_provider
        factory = APIRequestFactory()
        request = factory.put("/")
...
class TestMedicalRecordListSerializer:
    ...
    def test_serializer_with_multiple_records(self, medical_record_factory):
        ...
        for i, data in enumerate(serializer.data):
            ...
            assert (
                data["providerName"] == record.healthcare_provider.user.get_full_name()
            )
    ...
    def test_serializer_with_removed_provider(self, medical_record_factory):
        ...
        assert (
            serializer.data["providerName"]
            == record.healthcare_provider.user.get_full_name()
        )
...
class TestMedicalRecordDetailSerializer:
    ...
    def test_created_by_and_updated_by_names(
        self, medical_record_factory, provider_factory
    ):
    ...
    def test_medical_record_content_fields(self, medical_record_factory):
        ...
        assert (
            serializer.data["notes"]
            == "Patient presented with cough and fever for 3 days."
        )
        assert (
            serializer.data["prescriptions"]
            == "Amoxicillin 500mg TID for 7 days\nAcetaminophen PRN for fever"
        )
    ...
    def test_serializer_with_removed_provider(self, medical_record_factory):
        ...
        provider_details = serializer.data["providerDetails"]
        assert (
            provider_details["fullName"]
            == record.healthcare_provider.user.get_full_name()
        )
        assert (
            provider_details["licenseNumber"]
            == record.healthcare_provider.license_number
        )
