...
class TestSpecialityPermissions:
    ...
    def image_file(self):
        return SimpleUploadedFile("test.png", b"x", content_type="image/png")
    ...
    def test_unauthenticated_cannot_create(self, api_factory, create_payload):
        ...
        request = api_factory.post(
            "/specialities/", create_payload(), format="multipart"
        )
        response = view(request)
        assert response.status_code == status.HTTP_401_UNAUTHORIZED

    def test_regular_user_cannot_create(
        self, api_factory, create_payload, user_factory
    ):
        user = user_factory()
        view = SpecialityViewSet.as_view({"post": "create"})
        request = api_factory.post(
    ...
    def test_authenticated_can_list(
        self, api_factory, user_factory, speciality_factory
    ):
...
class TestSpecialityCreate:
    ...
    def test_create_duplicate_name(
        self, api_factory, create_payload, speciality_factory, admin_staff_factory
    ):
...
class TestSpecialityUpdate:
    ...
    def test_update_success(self, api_factory, speciality_factory, admin_staff_factory):
        ...
        request = api_factory.patch(
            f"/specialities/{speciality.pk}/", payload, format="json"
        )
        force_authenticate(request, user=admin_user.user)
        response = view(request, pk=speciality.pk)
    ...
    def test_regular_user_cannot_update(
        self, api_factory, speciality_factory, user_factory
    ):
        user = user_factory()
        speciality = speciality_factory()
