...
class AppointmentFactory(factory.django.DjangoModelFactory):
    ...
    appointment_end_datetime_utc = appointment_start_datetime_utc + timedelta(
        minutes=30
    )
...
class MedicalRecordFactory(factory.django.DjangoModelFactory):
    ...
    class Params:
        ...
        removed = factory.Trait(
            is_removed=True, removed_at=factory.Faker("past_datetime")
        )
    ...
    def _create(cls, model_class, *args, **kwargs):
        ...
        if not record.healthcare_provider.hospitals.filter(
